...
def toph(name,handel):	
	...
	while 1:
		...
		for quote in quotes:
			...
			sql1="SELECT * FROM %s WHERE subid=%s AND oj='toph'"
			val1 = (name,row[0].text,)
			mycursor.execute(sql1,val1)
			myresult = mycursor.fetchall()
			if(len(myresult)==0):
				sql="INSERT INTO %s (subid,dt,link,name,ver,oj) VALUES (%s,%s,%s,%s,%s,'toph')"
				val=(name,row[0].text,time,urlp,row[2].text,verdict1)
