...
def lightoj(name,handel,passwd): 
	...
	for quote in quotes:
		if cnt>0:
			...
			sql1="SELECT * FROM %s WHERE subid=%s AND oj='loj'"
			val1 = (name,sid,)
			mycursor.execute(sql1,val1)
			myresult = mycursor.fetchall()
			if(len(myresult)==0):
				sql="INSERT INTO %s (subid,dt,link,name,ver,oj) VALUES (%s,%s,%s,%s,%s,'loj')"
				val=(name,sid,tt,link,name,row[5].text)
